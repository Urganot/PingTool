...
internal static class LoggerTemplates
{
    ...
    public static void OutputStatistics(Pings pings)
    {
        ...
        Log.Information(
            $"Anzahl an gesendeten Pings: {pings.SentPings.Count()} => {pings.Count / Math.Max(1, pings.SentPings.Count()) * 100:F2}%");
        Log.Information(
            $"Anzahl an erfolgreichen Pings: {pings.SuccessfulPings.Count()} => {pings.SuccessfulPingsPercentage:F2}%");
        Log.Information(
            $"Anzahl an nicht erfolgreichen Pings: {pings.FailedPings.Count()} => {pings.FailedPingsPercent:F2}%");
        Log.Information(
            $"Anzahl an nicht gesendeten Pings: {pings.ExceptionPings.Count()} => {pings.ExceptionPingsPercentage:F2}");
